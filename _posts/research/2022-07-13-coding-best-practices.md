...
```
$ python3 gpt2
$ python3 gpt2 --learning_rate 1.0e-5 --num_train_epochs 100
$ python3 gpt2 --num_train_epochs 100
```
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "model_name",  # variable names without the -- prefix are positional, required, and don't take a flag name
    ...
    parser.add_argument(
        "--num_train_epochs",
        type=int,
        required=True  # you can require a flag
    )
...
ArgParse also supports lists as input values for parameters with the `extend` action and `nargs` argument:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_train_epochs",
        action="extend",
        nargs="*",  # any number of args (use "+" for at least 1 arg)
        type=int
    )
    args = parser.parse_args()
